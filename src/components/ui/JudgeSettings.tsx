...
// endpoint (Jev through OpenRouter, TypeSafe or Cloudflare; any self-hosted
// /v1/systemone; or the chat model as an uncalibrated stand-in). The note
// says where it is used and what leaves the machine.
...
export default function JudgeSettings() {
  ...
  return (
    <div className="border border-line rounded-xl px-3 py-2.5 bg-surface" data-judge-settings>
      <div className="flex items-center gap-1.5">
        <Scale size={14} strokeWidth={1.75} className="text-accent" />
        <span className="text-sm font-medium text-ink">{t('judge.title')}</span>
        <span className="text-ink-faint cursor-help" title={t('judge.where')} data-judge-info><Info size={13} strokeWidth={1.75} /></span>
      </div>
      <p className="text-2xs text-ink-faint mt-0.5 mb-2 leading-relaxed">{t('judge.where')}</p>
