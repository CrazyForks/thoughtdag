...
export default function AgentCwdChip() {
  ...
  const pick = async () => {
    ...
    // the host found it has no dialog after all (capabilities flipped): show the typed field now
    else if (window.desktopAgents?.capabilities?.nativePicker === false) rerender((n) => n + 1);
  };
